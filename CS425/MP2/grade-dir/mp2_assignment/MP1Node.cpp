...
void MP1Node::update_membership(Record_t* table, int id)
{
	MemberListEntry memberList;
	this->memberNode->memberList.clear();
	if(DEBUG_LOG)
		printf("Node %d, Reset membership \n", *((int*)(this->memberNode->addr.addr)));

	for(int i =0;i<par->EN_GPSZ;i++)
	{
		if (table[i].state == VALID)
		{
			short port = table[i].port;

			long heartbeat = table[i].heartbeat;

			long timestamp = table[i].timestamp;
			if (DEBUG_LOG)
				printf("Node %d, update_membership: adding  node :%d at time: %d\n",
				*((int*)(this->memberNode->addr.addr)),
				i,
				par->getcurrtime());

			MemberListEntry* entry = new MemberListEntry(i, port, heartbeat, timestamp);
