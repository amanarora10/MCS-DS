...
```{r}
df = read.csv("epa2017.csv")
#str(df)
df$type = as.factor(df$type)
model_fit = lm(CO2~ horse,  data = df)
plotcolors = c("purple", "red", "blue")
plot(CO2 ~ horse, data = df, col = plotcolors[type], pch =as.numeric(type))
legend("bottomright", c("both", "cars","trucks"), pch =c(1:3), col= plotcolors)
abline(model_fit, col = "green")
sum_model = summary(model_fit)
grid()

#Predict
newdata1 = data.frame( horse = 148)
result = predict(model_fit,newdata1, level = 0.9, interval = "prediction" )

```
  
* The regression line does not seem like a good fit with $R^2$ of: `r sum_model$r.squared`   

* Estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car` is: `r coef(model_fit)[2]`   

* The 90% prediction interval using this model for the `CO2` of a Subaru Impreza Wagon is (`r result[2] ` , `r result[3] `)
...
model_fit = lm(CO2~ horse + type,  data = df)
plot(CO2 ~ horse, data = df, col = plotcolors[type], pch =as.numeric(type))
legend("bottomright", c("both", "cars","trucks"), pch =c(1:3), col= plotcolors, lty = c(1:3))
grid()

sum_model = summary(model_fit)
int_both = coef(model_fit)[1]
int_car  = coef(model_fit)[1] + coef(model_fit)[3]
int_truck = coef(model_fit)[1] + coef(model_fit)[4]

slope = coef(model_fit)[2]

abline(int_both, slope, col = plotcolors[1], lty =1 )
abline(int_car, slope, col = plotcolors[2], lty = 2)
abline(int_truck, slope, col = plotcolors[3], lty =3)

#Predict
newdata1 = data.frame( horse = 148, type = "Both")
result = predict(model_fit,newdata1, level = 0.9, interval = "prediction" )
...
* The regression line does seem like a better fit that previous model  with $R^2$ of: `r sum_model$r.squared`   
...
slope_both = coef(model_fit)[2]
slope_car = coef(model_fit)[2] + coef(model_fit)[5]
slope_truck = coef(model_fit)[2] + coef(model_fit)[6]

abline(int_both, slope_both, col = plotcolors[1], lty =1 )
abline(int_car, slope_car, col = plotcolors[2], lty = 2)
abline(int_truck, slope_truck, col = plotcolors[3], lty =3)
...
* Estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car` is: `r slope_car`   
...
```{r}
model_add = lm(CO2 ~ horse+ type,  data = df)
model_int = lm(CO2 ~ horse*type,  data = df)

anova(model_add, model_int)
```
  
* Based on the p value (0.0059) from F test of additive and interaction  model, the iteraction model is a prefered model with  $\alpha = 0.10$
...
```{r}
model_add = lm(Blood ~ Age + Education + Income + Sex, data = df)
summary(model_add)

```

    
 R is using following ref. levels  
...
```{r}
model_int1  = lm(Blood ~ Age + Education + Income + Sex + Sex*Age + Sex*Income, data = df)
...
```
  ...
  * Based on the p-value of 0.11 we prefer the addtitive model
...
```{r}
model_int2  = lm(Blood ~ Age + Education + Income + Sex + Sex*Age + Sex*Income + Income*Age + Age*Income*Sex , data = df)
anova(model_add, model_int2)

sum = summary(model_int2)
```
  
  * Based on the p-value of 0.074 we prefer this  interaction model
...
```{r}
result = sum$coefficients["Age","Estimate"] + sum$coefficients["Age:Sexmale","Estimate"] +  sum$coefficients["Age:Incomelow","Estimate"]+
  sum$coefficients["Age:Incomelow:Sexmale","Estimate"]

```
  
  * The estimate of the change in average `Blood` for a one-unit increase in `Age` for a highly educated, low income, male patient is : `r result`
...
days_small = lm(Days ~ Pressure + Blood, data = hospital)
...
```{r}
anova(days_small,days_add )
```

  * The NULL hypothesis is $\beta_4 = \beta_5 = \beta_6 =  \beta_7 = 0$
  * The Alternate hypothesis at least one of coefficient $\beta_4,  \beta_5, \beta_6,   \beta_7$ is non zero.
  * The p value is  0.087
  * At $\alpha = 0.1$ we can reject the NULL hypothesis.
  * Thus We prefer the interaction model.
...
$\beta_3$ is the  true change in length of hospital stay in days for a 1 bpm increase in `Rate` for a patient with all other predictors remaining the same. Since its a additive model the affects of pressure and blood stay the same. 
...
result = t.test(values ~ groups, data = sim_data, var.equal = TRUE)
result$p.value
test = as.numeric(result$statistic)
```
...
for (i in 1: num_sims)
{
  ...
  #Store p/t value from LM
  lm_summary = summary(lm(values ~ groups, data = sim_data))
  lm_t[i] = lm_summary$coefficients["groupsB", "t value"]
  lm_p[i] = lm_summary$coefficients["groupsB", "Pr(>|t|)"]
  
  #Store p/t value from t test
  t_summary =  t.test(values ~ groups, data = sim_data, var.equal = TRUE)
  tt_p[i] =  t_summary$p.value
  tt_t[i] =  t_summary$statistic
...
```
  ...
  * The p values from t test and linear regression are now matching (after accounting for numerical precision.) this indicates two-sample $t$-test with equal variance is equivalent to a $t$-test for the coefficient of a single two-level factor variable (dummy variable) in a linear model.
...
The lm_tt and tt_t seems to have same magnitude but opposite signs. The sign of t indicates which group mean A or B is used as a reference. It seems for linear regresion (using lm)   and t test (using t.test ) they are swapped. We can  try swapping the reference level for lm and test again

```{r}
set.seed(19771212)

#Create a new column in data frame with swapped ref. level 
sim_data$groups_swap =  relevel(as.factor(sim_data$groups), ref = "B")

#Check ref. level is now B
levels(sim_data$groups_swap)
```


```{r}

#Rerun LM with swapped ref level (B is ref. for LM)
for (i in 1: num_sims)
{
  sim_data$values = rnorm(n, mean = 42, sd = 3.5) # simulate response data

  #Store p/t value from LM (using B as ref. level)
  lm_summary = summary(lm(values ~ groups_swap, data = sim_data))
  lm_t[i] = lm_summary$coefficients["groups_swapA", "t value"]
  lm_p[i] = lm_summary$coefficients["groups_swapA", "Pr(>|t|)"]
...
We can see below now the t value sign and magnitude match and all.equal is returning TRUE. It seems t.test is using B as ref level. 
