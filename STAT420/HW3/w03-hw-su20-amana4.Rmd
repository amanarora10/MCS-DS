...
par(mfrow=c(1,2))
hist(beta_hat_0,freq = FALSE,main = "Histogram of $\\beta_0$" )
...
```{r}
calc_pred_int <- function(fitted_model,newdata1, level =.95)
{
 # result_test = predict(fitted_model, newdata = newdata1, interval = "prediction")
  
  y_hat = predict(fitted_model, newdata = newdata1, interval = c("prediction"))
  p_value = (1-level)/2
  cat("pvalue", p_value, "\n")
  ...
  x= fitted_model$model[names(newdata1)][,1]
  x_mean = mean(x)
