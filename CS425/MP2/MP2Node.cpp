...
void MP2Node::init_trans(int transID, MessageType type, string key)
{
	...
	while ( !memberNode->mp2q.empty() ) {
		...
		case REPLY:
			if (msg.success)
			{
				...
				{
					if (msg_type == CREATE)
						log->logCreateSuccess(&memberNode->addr, true, msg.transID, string(trans[msg.transID].key), msg.value);
					if (msg_type == UPDATE)
						log->logUpdateSuccess(&memberNode->addr, true, msg.transID, string(trans[msg.transID].key), msg.value);
					if (msg_type == DELETE)
			...
			else
			{
				...
				{
					if (msg_type == CREATE)
						log->logCreateFail(&memberNode->addr, true, msg.transID, string(trans[msg.transID].key), msg.value);
					if (msg_type == UPDATE)
						log->logUpdateFail(&memberNode->addr, true, msg.transID, string(trans[msg.transID].key), msg.value);
					if (msg_type == DELETE)
