...
* From the plots above we can see mean test RMSE falls as the model complexity increase till model 6 (true model), after model 6 the decrease in test RMSE is minimal indicating  the true model may have only 6 predictors. Any test RMSE decrease in adding more parameters is negligible. 


* As we can see from plot the method does not always select the correct model (model 6). In fact its skewed towards choosing models with higher number of parameters over 6. The method does do well in rejecting the models with less parameters than true model but does tend to select a model with higher parameters than true model since they tend to do better in terms of RMSE. 

* The variance also tends to increase the test/train RMSE. Moreover we tend to select incorrect model with increased variance. As an example with variance of 1 and 2 the method does not pick any model with less than 6 predictors but at variance of 4  the method starts to select  models with 4 and 5 predictors as best model as well. 
...
for(sigma_idx in 1:length(sigmas))
{
  ...
  plot(results[,sigma_idx,1], x=beta1_list, type = "l", col = "green", main = title, ylab = "Power", xlab = "beta1")
