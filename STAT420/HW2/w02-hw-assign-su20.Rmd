...
```
Since the predictor variable is in the range of data, the prediction is interpolation based and likely reliable.   
...
p + geom_abline(intercept=cat_model$coefficients[1], slope = cat_model$coefficients[2], color = "firebrick", size = 1)

#p + geom_abline(cat_model$coefficients[1], cat_model$coefficients[2])
...
hist(beta_hat_1)
```
The distribtion pf estimate of the slope $\hat{\beta_1}$ looks noramal with mean as true slope (-3)    
...
#Humidity
summ_humidity= summary(model_humidity)
summ_humidity$r.squared
...
Based on results the temperature  predictor seems to be most useful since it has highest $R^2$ thus can explain the  the variance of ozone best than other predictors. 
